...
omit_sites = None # tuple of site idices to exclude from lower energy (even) sites
...
util_min_Wan = find_optimal_subspace(
    model, outer_states, inner_states, k_mesh=None, full_mesh=False, tol=2e-4, iter_num=100)
...
U, _ = find_min_unitary(model, M, iter_num=100, eps=1e-5, print_grad=True)

u_min = np.zeros(util_min_Wan.shape, dtype=complex)
for kx in range(nkx):
    for ky in range(nky):
        u_min[kx, ky] = U[kx, ky] @ util_min_Wan[kx, ky]
